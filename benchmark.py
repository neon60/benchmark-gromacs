...
def runOneTest(testName, options):
    ...
    if testName == 'adh_dodec_stream_commands':
        os.system(f'/usr/local/gromacs/bin/gmx mdrun -pin on -nsteps {steps} -resetstep 10000 -ntmpi 1 -ntomp 48 -notunepme -noconfout -nb gpu -bonded cpu -pme gpu -v -gpu_id 0 -s adh_dodec/topol.tpr')
    elif testName == 'adh_dodec_mi100_commands':
        os.system(f'/usr/local/gromacs/bin/gmx mdrun -pin on -nsteps {steps} -resetstep 10000 -ntmpi 2 -ntomp 28 -notunepme -noconfout -nb gpu -bonded cpu -pme gpu -npme 1 -v -gpu_id 0 -s adh_dodec/topol.tpr')
